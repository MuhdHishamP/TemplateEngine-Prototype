...
const CardTwo = ({ key, productData, storeCurrency, dis_price, retail_price, discount, cardData }) => {
    ...
    return (
        <>
            <div key={key} className={`flex flex-col justify-center md:mt-2 items-center bg-[#fafafa] dark:bg-[#1a1a1a] mt-4`}>
                <div style={{
                    "backgroundColor": cardData ? cardData[0]?.cardTwo?.backgroundColor : null,
                    "borderRadius": cardData ? cardData[0]?.cardTwo?.brRadius + "%" : null,
                    "padding": `${cardData ? cardData[0]?.cardTwo?.boxPadding : null}%`,
                    "box-shadow": cardData ? cardData[0]?.cardTwo?.boxShadow : null,
                }}
                    className={`dark:bg-[#2a2a2a] mx-2 p- shadow hover:scale-105 hover:shadow-xl duration-500`}>
                    <div className="block">

                        {cardData ? cardData[0]?.cardTwo?.isScroll === false ? (
                            <>
                                <Swiper
                                    slidesPerView="auto"
                                    spaceBetween={10}
                                    ...
                                    style={{
                                        ...
                                        "border-radius": `${cardData ? cardData[0]?.cardTwo?.brRadiusImage : null}%`,
                                        "padding": `${cardData ? cardData[0]?.cardTwo?.imagePadding : null}%`,
                                        "border-radius": `${cardData ? cardData[0]?.cardTwo?.brRadiusImage : null}%`,
                                        "box-shadow": cardData ? cardData[0]?.cardTwo?.imageShadow : null,
                                    }}
                                    //navigation={true}
                                    effect="fade"
                            ...
                            <>
                                <div className="p-2">
                                    <img style={{
                                        ...
                                        src={productData?.images[0].image} className="object-cover h-64 w-full" />
                                </div>
                            </>
                        ) : null}
                    </div>
                    ...
                    <div className="px-4 py-3 cursor-pointer">
                        <a href={'/product/' + productData?.name + "/" + productData?.id} target="_blank" rel="noopener noreferrer">
                            <span className="mr-3 md:text-sm text-2xl font-semibold"
                                style={{ "color": cardData ? cardData[0]?.cardTwo?.titleColor : null }} >
                            ...
                            <p className="md:text-sm text-lg text-[#5b5b5b] dark:text-[#dadada] block"
                                style={{ "color": cardData ? cardData[0]?.cardTwo?.catColor : null }}>
                                {productData?.cat?.name.substring(0, 25)} {productData?.cat?.name.length >= 25 && '...'}
                            </p>
                            <div className="flex items-center">
                                {productData?.discount_price ? (
                                    <>
                                        <p style={{ "color": cardData ? cardData[0]?.cardTwo?.priceColor : null }}
                                            className="md:text-sm text-xl font-semibold my-3">{storeCurrency} {dis_price}</p>
                                        <del>
                                            <p style={{ "color": cardData ? cardData[0]?.cardTwo?.disDelColor : null }}
                                ...
                                {discount === "none" ? (
                                    ...
                                    <>
                                        <p style={{ "color": cardData ? cardData[0]?.cardTwo?.disColor : null }}
